from fastapi import APIRouter, Depends, Query
from sqlalchemy.orm import Session
from app.database import get_db
from app.schemas.task import TaskCreate, TaskResponse, TaskAcceptResponse
from app.services.task_service import (
    create_task, get_tasks_in_radius, accept_task,
    complete_task, _task_to_response,
)
from app.services.auth_service import get_current_user
from app.models.user import User

router = APIRouter(prefix="/tasks", tags=["Tasks"])

# Default to Jaipur, Rajasthan for local testing
JAIPUR_LAT = 26.9124
JAIPUR_LON = 75.7873


@router.post("", response_model=TaskResponse, status_code=201)
def post_task(
    payload: TaskCreate,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    return create_task(db, payload, current_user)


@router.get("", response_model=list[TaskResponse])
def list_tasks(
    lat: float = Query(default=JAIPUR_LAT, description="Viewer latitude"),
    lon: float = Query(default=JAIPUR_LON, description="Viewer longitude"),
    category: str | None = Query(default=None),
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    """Only returns tasks where the viewer is within the task's set radius."""
    return get_tasks_in_radius(db, lat, lon, category)


@router.post("/{task_id}/accept", response_model=TaskAcceptResponse)
def accept(
    task_id: str,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    task = accept_task(db, task_id, current_user)
    return TaskAcceptResponse(
        task_id=task.id,
        accepted_by=current_user.name,
        status=task.status,
        chat_unlocked=True,
    )


@router.post("/{task_id}/complete", response_model=TaskResponse)
def complete(
    task_id: str,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    task = complete_task(db, task_id, current_user)
    return _task_to_response(task)
